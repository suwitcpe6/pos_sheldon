...
Public Class F_MAS_REASON
    ...
    Dim data_H As MAS_REASON_Models.REASO_Data
    Private Sub F_MAS_REASON_Activated(sender As Object, e As EventArgs) Handles Me.Activated
...
#Region "Event"
    Public Sub SetEvent() Implements CL_interface.DefultFoundation.SetEvent
        'text
        AddHandler Me.RSCODE.TextChanged, AddressOf EV_Text_Changed
        AddHandler Me.RSNAME.TextChanged, AddressOf EV_Text_Changed
...
#Region "Implements"
    Public Sub EnableControl(Value As Boolean) Implements CL_interface.DefultFoundation.EnableControl
        ...
        Select Case Me.UI_STAT
            Case CL_interface.MemberFormStatus.UINew
                RSCODE.US_Enable = False
                RSNAME.US_Enable = Value
    ...
    Public Sub Load_MAS() Implements CL_interface.DefultFoundation.Load_MAS
        Try
            ...
            dt.Columns.Add("name")
            dt.Rows.Add("RSCODE", "รหัสเหตุผล")
            dt.Rows.Add("RSNAME", "ชื่อเหตุผล")
    ...
    Public Sub Load_Data(value1 As String, value2 As String, value3 As Object) Implements CL_interface.DefultFoundation.Load_Data
        ...
        Try
            ...
            Dim Link As String
            Dim json_Response As String
            Link = String.Format(Center.Get_API.REASON_Select, value1)
            json_Response = conn_api.GETSON(Link)
            Dim ret As New MAS_REASON_Models.value_data
            ret = (JsonConvert.DeserializeObject(Of MAS_REASON_Models.value_data)(json_Response))
            If ret.Respon.Result Then
                data_H = ret.Data(0)

                ID.Text = data_H.ID
                RSCODE.Text = data_H.RSCODE
                RSNAME.Text = data_H.RSNAME
    ...
    Public Sub Gencolumn() Implements CL_interface.DefultFoundation.Gencolumn
        ...
        Col = New DataGridViewTextBoxColumn
        Col.HeaderText = "รหัสเหตุผล"
        Col.Width = 50
        Col.Visible = True
        Col.ReadOnly = read
        Col.DataPropertyName = "RSCODE"
        Col.Name = "RSCODE"
        Me.dgv_scr.Columns.Add(Col)
        ...
        Col = New DataGridViewTextBoxColumn
        Col.HeaderText = "ชื่อเหตุผล"
        Col.Width = 50
        Col.Visible = True
        Col.ReadOnly = read
        Col.DataPropertyName = "RSNAME"
        Col.Name = "RSNAME"
        Me.dgv_scr.Columns.Add(Col)
    ...
    Public Function Checkdata() As Boolean Implements CL_interface.DefultFoundation.Checkdata
        ...
        'If RSCODE.Text = "" Then
        '    RSCODE.UCSet_err("ไม่ได้ใส่รหัสเหตุผล")
        '    msg = "- ไม่ได้ใส่รหัสเหตุผล"
        'End If
        If RSNAME.Text = "" Then
            RSNAME.UCSet_err("ไม่ได้ใส่ชื่อเหตุผล")
            msg &= vbNewLine & "- ไม่ได้ใส่ชื่อเหตุผล"
        End If
        If msg <> "" Then
    ...
    Public Function FSave(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FSave
        Try
            ...
            If Save_H() Then
                ...
                Dim link As String = String.Format(Center.Post_API.MAS_REASON_Save, UI_STAT)
                Dim json_Response As String = New API(Center.Data_Config.API_SRV).POSTResponse(link, json, "POST")
                Dim retValue As MAS_REASON_Models.value_data = JsonConvert.DeserializeObject(Of MAS_REASON_Models.value_data)(json_Response)
    ...
    Public Function FSearch(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FSearch
        Try
            ...
            Dim CODE As String = ""
            Dim NAME As String = ""
            If cbo_search.ComboBox.SelectedValue = "RSCODE" Then
                CODE = txt_scr.Text.Trim
            ElseIf cbo_search.ComboBox.SelectedValue = "RSNAME" Then
            ...
            Link = String.Format(Center.Get_API.REASON_List, CODE, NAME, Center.Limit_Search)
            json_Response = conn_api.GETSON(Link)
            Dim ret As New MAS_REASON_Models.value_List
            ret = (JsonConvert.DeserializeObject(Of MAS_REASON_Models.value_List)(json_Response))
            If ret.Respon.Result Then
    ...
    Public Function FDelete(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FDelete
        Try
            If Me.dgv_scr.RowCount < 0 Or RSCODE.Text = "" Then
            ...
            Dim link As String = String.Format(Center.Post_API.MAS_REASON_DELETE, ID.Text)
...
#Region "SAVE"
    Private Function Save_H() As Boolean
        data_H = New MAS_REASON_Models.REASO_Data
        data_H.ID = Me.ID.Text
        data_H.RSCODE = RSCODE.Text
        data_H.RSNAME = RSNAME.Text
