...
Public Class F_Mas_Product_Group
    ...
    Dim data_H As Mas_Product_Group_Models.PRODUCT_Group
...
#Region "Event"
    Public Sub SetEvent() Implements CL_interface.DefultFoundation.SetEvent
        'text
        AddHandler Me.PGCODE.TextChanged, AddressOf EV_Text_Changed
        AddHandler Me.PGNAME.TextChanged, AddressOf EV_Text_Changed
    ...
    Private Sub EV_Btn_Click(sender As Object, e As EventArgs)
        Try
            Select Case sender.name
                ...
                Case btn_close.Name : FExit() : Btn_menu.Hid_Button(ToolStrip_top, Me.UI_STAT)
                Case Btn_last.Name
...
#Region "Implements"
    Public Sub EnableControl(Value As Boolean) Implements CL_interface.DefultFoundation.EnableControl
        ...
        Select Case Me.UI_STAT
            Case CL_interface.MemberFormStatus.UINew
                PGCODE.US_Enable = False
                PGNAME.US_Enable = Value
    ...
    Public Sub Load_MAS() Implements CL_interface.DefultFoundation.Load_MAS
        Try
            ...
            dt.Columns.Add("name")
            dt.Rows.Add("PGCODE", "รหัสกลุ่ม")
            dt.Rows.Add("PGNAME", "ชื่อกล่ม")
    ...
    Public Sub Load_Data(value1 As String, value2 As String, value3 As Object) Implements CL_interface.DefultFoundation.Load_Data
        ...
        Try
            ...
            Dim Link As String
            Dim json_Response As String
            Link = String.Format(Center.Get_API.MAS_PRODUCT_Group_Select, value1)
            json_Response = conn_api.GETSON(Link)
            Dim ret As New Mas_Product_Group_Models.value_data
            ret = (JsonConvert.DeserializeObject(Of Mas_Product_Group_Models.value_data)(json_Response))
            If ret.Respon.Result Then
                data_H = ret.Data(0)

                ID.Text = data_H.ID
                PGCODE.Text = data_H.PGCODE
                PGNAME.Text = data_H.PGNAME
    ...
    Public Sub Gencolumn() Implements CL_interface.DefultFoundation.Gencolumn
        ...
        Col = New DataGridViewTextBoxColumn
        Col.HeaderText = "รหัสกลุ่ม"
        Col.Width = 50
        Col.Visible = True
        Col.ReadOnly = read
        Col.DataPropertyName = "PGCODE"
        Col.Name = "PGCODE"
        Me.dgv_scr.Columns.Add(Col)

        Col = New DataGridViewTextBoxColumn
        Col.HeaderText = "ชื่อกลุ่ม"
        Col.Width = 50
        Col.Visible = True
        Col.ReadOnly = read
    ...
    Public Function Checkdata() As Boolean Implements CL_interface.DefultFoundation.Checkdata
        ...
        'If PGCODE.Text = "" Then
        '    PGCODE.UCSet_err("ไม่ได้ใส่รหัสกลุ่ม")
        '    msg = "- ไม่ได้ใส่รหัสกลุ่ม"
        'End If
        If PGNAME.Text = "" Then
            PGNAME.UCSet_err("ไม่ได้ใส่ชื่อกลุ่ม")
            msg &= vbNewLine & "- ไม่ได้ใส่ชื่อกลุ่ม"
        End If
        If msg <> "" Then
    ...
    Public Function FSave(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FSave
        Try
            ...
            If Save_H() Then
                ...
                Dim link As String = String.Format(Center.Post_API.Mas_Product_Group_Save, UI_STAT)
    ...
    Public Function FSearch(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FSearch
        Try
            ...
            If cbo_search.ComboBox.SelectedValue = "PGCODE" Then
            ...
            Link = String.Format(Center.Get_API.MAS_PRODUCT_Group_List, UnitCODE, UnitNAME)
            json_Response = conn_api.GETSON(Link)
    ...
    Public Function FDelete(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FDelete
        Try
            If Me.dgv_scr.RowCount < 0 Or PGCODE.Text = "" Then
            ...
            Dim link As String = String.Format(Center.Post_API.Mas_PRODUCT_Group_DELETE, ID.Text)
...
#Region "SAVE"
    Private Function Save_H() As Boolean
        data_H = New Mas_Product_Group_Models.PRODUCT_Group
        data_H.ID = Me.ID.Text
        data_H.PGCODE = PGCODE.Text
        data_H.PGNAME = PGNAME.Text
