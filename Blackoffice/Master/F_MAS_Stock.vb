...
Public Class F_MAS_Stock
    ...
    Dim data_H As Mas_Stock_Models.Stock
...
#Region "Event"
    Public Sub SetEvent() Implements CL_interface.DefultFoundation.SetEvent
        'text
        AddHandler Me.STCODE.TextChanged, AddressOf EV_Text_Changed
        AddHandler Me.STNAME.TextChanged, AddressOf EV_Text_Changed
...
#Region "Implements"
    ...
    Public Sub Load_MAS() Implements CL_interface.DefultFoundation.Load_MAS
        Try
            ...
            dt.Columns.Add("name")
            dt.Rows.Add("STCODE", "รหัสคลัง")
            dt.Rows.Add("STNAME", "ชื่อคลัง")
    ...
    Public Sub Load_Data(value1 As String, value2 As String, value3 As Object) Implements CL_interface.DefultFoundation.Load_Data
        ...
        Try
            ...
            Dim Link As String
            Dim json_Response As String
            Link = String.Format(Center.Get_API.MAS_STOCK_Select, value1)
            json_Response = conn_api.GETSON(Link)
            Dim ret As New Mas_Stock_Models.value_data
            ret = (JsonConvert.DeserializeObject(Of Mas_Stock_Models.value_data)(json_Response))
            If ret.Respon.Result Then
                data_H = ret.Data(0)

                ID.Text = data_H.ID
                STCODE.Text = data_H.STCODE
                STNAME.Text = data_H.STNAME
    ...
    Public Sub Gencolumn() Implements CL_interface.DefultFoundation.Gencolumn
        ...
        Col = New DataGridViewTextBoxColumn
        Col.HeaderText = "รหัสคลัง"
        Col.Width = 50
        Col.Visible = True
        Col.ReadOnly = read
        Col.DataPropertyName = "STCODE"
        Col.Name = "STCODE"
        Me.dgv_scr.Columns.Add(Col)

        Col = New DataGridViewTextBoxColumn
        Col.HeaderText = "ชื่อคลัง"
        Col.Width = 50
        Col.Visible = True
        Col.ReadOnly = read
    ...
    Public Function Checkdata() As Boolean Implements CL_interface.DefultFoundation.Checkdata
        ...
        If STCODE.Text = "" Then
            STCODE.UCSet_err("ไม่ได้ใส่รหัสคลัง")
            msg = "- ไม่ได้ใส่รหัสคลัง"
        End If
        If STNAME.Text = "" Then
            STNAME.UCSet_err("ไม่ได้ใส่ชื่อคลัง")
            msg &= vbNewLine & "- ไม่ได้ใส่ชื่อคลัง"
        End If
        If msg <> "" Then
    ...
    Public Function FSave(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FSave
        Try
            ...
            If Save_H() Then
                ...
                Dim link As String = String.Format(Center.Post_API.Mas_STOCK_Save, UI_STAT)
    ...
    Public Function FSearch(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FSearch
        Try
            ...
            If cbo_search.ComboBox.SelectedValue = "STCODE" Then
            ...
            Link = String.Format(Center.Get_API.MAS_STOCK_List, UnitCODE, UnitNAME)
            json_Response = conn_api.GETSON(Link)
    ...
    Public Function FDelete(Optional obj As Object = Nothing) As CL_interface.Answer Implements CL_interface.DefultFoundation.FDelete
        Try
            If Me.dgv_scr.RowCount < 0 Or STCODE.Text = "" Then
            ...
            Dim link As String = String.Format(Center.Post_API.MAS_Stock_DELETE, ID.Text)
            Dim json_Response As String = New API(Center.Data_Config.API_SRV).POSTResponse(link, "", "Delete")
            Dim retValue As Mas_Stock_Models.value_data = JsonConvert.DeserializeObject(Of Mas_Stock_Models.value_data)(json_Response)
...
#Region "SAVE"
    Private Function Save_H() As Boolean
        data_H = New Mas_Stock_Models.Stock
        data_H.ID = Me.ID.Text
        data_H.STCODE = STCODE.Text
        data_H.STNAME = STNAME.Text
